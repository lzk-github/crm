...
public class WeixinUtil {
    ...
    private static final String CREATE_DEPT_URL =
            "https://qyapi.weixin.qq.com/cgi-bin/department/create?access_token={0}";

    private static final String DELETE_DEPT_URL =
    ...
    private static final String SEND_TEXT_MESSAGE_URL =
    ...
    @Value("${weixin.addressBook.secret}")
    ...
    private LoadingCache<String,String> accessTokenCache = CacheBuilder.newBuilder()
            ...
            .build(new CacheLoader<String, String>() {
                ...
                public String load(String key) throws Exception {
                    String url = null;
                    if(TOKEN_TYPE_ADDRESS_BOOK.equals(key)) {
                    ...
                    if("0".equals(errorCode)) {
                        ...
                        throw new WeixinException("获取access_token异常,错误码=" +
                                errorCode + " ;错误信息=" +
                                errorCodeMap.get(Integer.valueOf(errorCode)));
                    }
                }
            });
     ...
     * @param deptName
     */
    public void createDept(int deptId,int parentId,String deptName) {
        ...
        String result = sendPostRequest(url,paramMap);
        Map<String,Object> map = JSON.parseObject(result,HashMap.class);
        String errcode = map.get("errcode").toString();
        if(!"0".equals(errcode)) {
            throw new WeixinException("创建部门异常,错误码=" + errcode + "; 错误详情:"
                    + errorCodeMap.get(Integer.parseInt(errcode)));
        }
    ...
    public void deleteByDeptId(Integer deptId) {
        String url = MessageFormat.format(DELETE_DEPT_URL,getAccessToken(TOKEN_TYPE_ADDRESS_BOOK),deptId.toString());
        String result = sendGetRequest(url);
        //JSON -> Map
        Map<String,Object> map = JSON.parseObject(result, HashMap.class);
        String errorCode = map.get("errcode").toString();
        if(!errorCode.equals("0")) {
            throw new WeixinException("删除部门异常，错误码:" + errorCode + " 错误原因：" + errorCodeMap.get(Integer.parseInt(errorCode)));
        }
    }
     ...
     * @param deptIds 所属部门列表（可以属于多个部门）
     * @param mobile 手机号码
     */
    public void addAccount(int accountId, String accountName, Integer[] deptIds, String mobile) {
        ...
        String result = sendPostRequest(url,map);

        Map<String,Object> resultMap = JSON.parseObject(result,HashMap.class);
        String errorCode = resultMap.get("errcode").toString();
        if(!"0".equals(errorCode)) {
            throw new WeixinException("创建用户异常，错误码：" + errorCode +
                    "; 错误详细信息：" + errorCodeMap.get(new Integer(errorCode)));
        }
    ...
    public void deleteAccount(Integer accountId) {
        String url = MessageFormat.format(DELETE_ACCOUNT_URL,getAccessToken(TOKEN_TYPE_ADDRESS_BOOK),accountId.toString());
        String result = sendGetRequest(url);
        Map<String,Object> resultMap = JSON.parseObject(result,HashMap.class);
        String errorCode = resultMap.get("errcode").toString();
        if(!"0".equals(errorCode)) {
            throw new WeixinException("删除用户异常，错误码：" + errorCode +
                    "; 错误详细信息：" + errorCodeMap.get(new Integer(errorCode)));
        }
    }
     ...
     * @param userIds
     */
    public void sendMessageToUser(String message,String... userIds) {
        String formatUserIds = String.join("|",userIds);
        sendMessage(message,formatUserIds,null);
    }
     ...
     * @param deptIds
     */
    public void sendMessageToDept(String message,String... deptIds) {
        String formatDeptIds = String.join("|",deptIds);
        sendMessage(message,null,formatDeptIds);
    }

    /**
     * 内部方法,提供给员工或部门发送信息的实现方法
     * @param message 信息内容
     * @param formatUserIds 格式化后的员工id
     * @param formatDeptIds 格式化后的部门id
     */
    private void sendMessage(String message, String formatUserIds, String formatDeptIds) {
        String url = MessageFormat.format(SEND_TEXT_MESSAGE_URL,getAccessToken(TOKEN_TYPE_NORMAL));

        Map<String,Object> textMap = Maps.newHashMap();     //发送信息的map
        textMap.put("content",message);

        Map<String,Object> param = Maps.newHashMap();       //post请求传参的map

        if(formatUserIds != null) {
            param.put("touser",formatUserIds);
        }
        if(formatDeptIds != null) {
            param.put("toparty",formatDeptIds);
        }
        ...
        String errorCode = resultMap.get("errcode").toString();
        if(!"0".equals(errorCode)) {
            throw new WeixinException("发送消息异常，错误码：" + errorCode +
                    "; 错误详细信息：" + errorCodeMap.get(new Integer(errorCode)));
        }
    ...
    private String sendPostRequest(String url, Object obj) {
        String json = JSON.toJSONString(obj);   //把传入的object对象转成json类型
    ...
    private String sendGetRequest(String url) {
        ...
        try {
            ...
            throw new WeixinException("发起get请求AccessToken异常,请求url:" + url);
        }
    }
