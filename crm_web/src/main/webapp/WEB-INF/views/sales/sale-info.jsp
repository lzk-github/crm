                ...
                <div class="box-body no-padding">
                    <table class="table">
                        ...
                        <tr>
                            ...
                            <td>${chance.customer.trade}</td>
                            <td class="td_title">客户来源</td>
                            <td>${chance.customer.source}</td>
                            <td class="td_title">级别</td>
                            <td class="star">${chance.customer.level}</td>
                        </tr>
                        <c:if test="${not empty chance.customer.address}">
                            <tr>
                                <td class="td_title">地址</td>
                                <td colspan="5">${chance.customer.address}</td>
                            </tr>
                        </c:if>
                        <c:if test="${not empty chance.customer.mark}">
                            <tr>
                                <td class="td_title">备注</td>
                                <td colspan="5">${chance.customer.mark}</td>
                            </tr>
                        </c:if>
                    </table>
                </div>
                                    ...
                                    <c:if test="${task.state == 0}">
                                        <li>
                                            ...
                                            <c:if test="${not empty task.custName}">
                                                <a href="/customer/my/${task.customerId}"><i class="fa fa-user-o"></i> ${task.custName}</a>
                                            </c:if>
                                            <div class="tools">
            ...
            <div class="modal fade" id="changeProgressModal">
                <div class="modal-dialog">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <form method="post" action="/sales/my/update/progress/${chance.id}" id="updateProgressForm">
                                <%--<input type="hidden" name="chanceId" value="${chance.id}">--%>
    ...
    $(function () {
        ...
        $("#delBtn").click(function () {
            layer.confirm("确定要删除该机会吗?",function () {
                window.location.href = "/sales/del/" + chanceId;
            });
        });
        //添加跟进记录
